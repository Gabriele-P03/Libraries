...
jpl::_security::_cypher::_symmetric::DES::DES(std::string key){
...
void jpl::_security::_cypher::_symmetric::DES::rotate(bool *&_l, bool *&_r, unsigned short round){
...
void jpl::_security::_cypher::_symmetric::DES::_pc2(bool *_l, bool *_r, unsigned short round)
{
    ...
    for (unsigned short i = 0; i < DES_JPL_PC2_KEY_SIZE; i++){
        unsigned short index = jpl::_security::_cypher::_symmetric::_des::pc2_matrix[i];
        buffer[i] = _l_r[index-1];
    }
    this->sub_keys_bits.push_back(buffer);
}

bool* jpl::_security::_cypher::_symmetric::DES::mangler(bool* buffer, unsigned short round){
    ...
    for(unsigned short i = 0; i < 48; i++){
        unsigned short index = jpl::_security::_cypher::_symmetric::_des::exp_matrix[i];
        buffer_exped[i] = buffer[index-1];
    }

    bool* bits_xor = jpl::_utils::_bits::_xor_bits(buffer_exped, key_round, 48);
    delete[] buffer_exped;

    //Execute S boxes
    bool *post_p = new bool[32], *pre_p = new bool[32];;
    for(unsigned short i = 0; i < 8; i++){
        const unsigned short* box = jpl::_security::_cypher::_symmetric::_des::s_boxes[i];
    ...
    for(unsigned short j = 0; j < 32; j++){
        unsigned short index = jpl::_security::_cypher::_symmetric::_des::p_matrix[j];
...
std::string* jpl::_security::_cypher::_symmetric::DES::ecb_encrypt(std::string plain_text){
    ...
    do{
        ...
        bool* plain_block_bits = jpl::_utils::_conversions::cc_bits(plain_block.c_str(), 8);
        //Let's encrypt the sub block
        bool* cyphered_bits = this->enc_sub_block(plain_block_bits);

        const char* bits_cc_ = jpl::_utils::_conversions::bits_cc(cyphered_bits, 64);
...
bool* jpl::_security::_cypher::_symmetric::DES::enc_sub_block(bool* plain_block_bits){
        //let's execute the initial permutation on the plain block
        bool* ip_buffer = new bool[64];
        for(unsigned short i = 0; i < 64; i++){
            ip_buffer[i] = plain_block_bits[jpl::_security::_cypher::_symmetric::_des::ip_matrix[i]-1];
        }
        ...
        for(unsigned short i = 0; i < DES_JPL_ROUND; i++){
            ...
            bool* bits_xored = jpl::_utils::_bits::_xor_bits(_left, manglered, 32);
        ...
        for(unsigned short i = 0; i < 64; i++){
            unsigned short index = jpl::_security::_cypher::_symmetric::_des::fp_matrix[i];
...
std::string* jpl::_security::_cypher::_symmetric::DES::ecb_decrypt(std::string cyphered_text){
    if(!jpl::_security::_cypher::checkError()){
        return nullptr;
    }
    //Reverse keys
...
void jpl::_security::_cypher::_symmetric::DES::set_CBC_IV(std::string IV){
    ...
    if(IV.size() != 8){
        jpl::_security::_cypher::_error_cypher_jpl = CYPHER_JPL_IV_SIZE_ERROR;
    }else{
        this->iv_initialized = true;
        this->IV = jpl::_utils::_conversions::cc_bits(IV.c_str(), 8);
    }
}

std::string* jpl::_security::_cypher::_symmetric::DES::cbc_encrypt(std::string plain_text){

    if(!this->iv_initialized){
        jpl::_security::_cypher::_error_cypher_jpl = CYPHER_JPL_IV_UNINIT_ERROR;
    }
    ...
    do{
        ...
        bool* plain_block_bits = jpl::_utils::_conversions::cc_bits(plain_block.c_str(), 8);
        //Let's encrypt the sub block
        bool* plain_block_bits_ived = jpl::_utils::_bits::_xor_bits(plain_block_bits, round_v, 64);
        bool* cyphered_bits = this->enc_sub_block(plain_block_bits_ived);

        char* buffer = jpl::_utils::_conversions::bits_cc(cyphered_bits, 64);
...
std::string* jpl::_security::_cypher::_symmetric::DES::cbc_decrypt(std::string cyphered_text){
    ...
    do{
        ...
        bool* current_block_bits = jpl::_utils::_conversions::cc_bits(current_block, 8);
        bool* enc_sub_block_ = this->enc_sub_block(current_block_bits);
        bool* enc_sub_block_ived = jpl::_utils::_bits::_xor_bits(enc_sub_block_, current_iv, 64);

        const char* bits_cc_ = jpl::_utils::_conversions::bits_cc(enc_sub_block_ived, 64);
...
jpl::_security::_cypher::_symmetric::DES::~DES(){
