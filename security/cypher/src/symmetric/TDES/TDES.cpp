...
jpl::_security::_cypher::_symmetric::TDES::TDES(std::string key1, std::string key2, std::string key3){

    this->first = new jpl::_security::_cypher::_symmetric::DES(key1);
    this->second = new jpl::_security::_cypher::_symmetric::DES(key2);
    this->third = new jpl::_security::_cypher::_symmetric::DES(key3);
}



//ECB part
std::string* jpl::_security::_cypher::_symmetric::TDES::ecb_encrypt(std::string plain_text){
...
std::string* jpl::_security::_cypher::_symmetric::TDES::ecb_decrypt(std::string cyphered_text){
...
//CBC part
std::string* jpl::_security::_cypher::_symmetric::TDES::cbc_encrypt(std::string plain_text){
    ...
    do{
        ...
        bool* cc_sub_block_bits = jpl::_utils::_conversions::cc_bits(cc_sub_block, 8);
        bool* cc_sub_block_bits_ived = jpl::_utils::_bits::_xor_bits(cc_sub_block_bits, round_v, 64);
        const char* cc_sub_block_ived = jpl::_utils::_conversions::bits_cc(cc_sub_block_bits_ived, 64);
        ...
        delete[] round_v;
        round_v = jpl::_utils::_conversions::cc_bits(buffer->c_str(), 8);
...
std::string* jpl::_security::_cypher::_symmetric::TDES::cbc_decrypt(std::string cyphered_text){
    if(!this->iv_initialized){
        jpl::_security::_cypher::_error_cypher_jpl = CYPHER_JPL_IV_UNINIT_ERROR;
    }
    ...
    do{
        ...
        bool*  buffer_bits = jpl::_utils::_conversions::cc_bits(buffer->c_str(), 8);
        delete buffer;
        bool* buffer_bits_ived = jpl::_utils::_bits::_xor_bits(round_v, buffer_bits, 64);
        delete[] buffer_bits;
        const char* cc_buffer_ived = jpl::_utils::_conversions::bits_cc(buffer_bits_ived, 64);
        delete[] buffer_bits_ived;
        std::string* buffer_ived = new std::string(cc_buffer_ived, 8);

        //Let sub_block_ived become the next round_v
        delete[] round_v;
        round_v = jpl::_utils::_conversions::cc_bits(sub_block.c_str(), 8);
...
void jpl::_security::_cypher::_symmetric::TDES::set_CBC_IV(std::string IV){
    this->IV = jpl::_utils::_conversions::cc_bits(IV.c_str(), 8);
    this->iv_initialized = true;
}

jpl::_security::_cypher::_symmetric::TDES::~TDES(){
