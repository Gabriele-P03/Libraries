...
struct jpl::_socket::ClientListController::Permission{
...
jpl::_socket::ClientListController::ClientListController(){

    this->checkPermission = false;
}

bool jpl::_socket::ClientListController::isClientAllowed(jpl::_socket::Client* client){
    ...
    if(this->permissions != nullptr){
        if(this->permissions->size() > 0){
            for(ssize_t i = 0; i < this->permissions->size(); i++ ){
                ...
                std::string ip_client = jpl::_socket::getClientIp(client);
...
void jpl::_socket::ClientListController::addNewClient(jpl::_socket::Client* &client){
...
void jpl::_socket::ClientListController::loadPermission(std::ifstream* stream){
    ...
    while(std::getline(*stream, buffer)){
        ...
        this->permissions->push_back(jpl::_socket::ClientListController::Permission(name_client, ip, root));
